...
*/
function createTables(sequelize, callback) {
...
function getCredit(credit, sequelize, config, callback) {
  ...
  var sql = 'Select * from Credit where description = :description and wallet = :wallet ;';

  sequelize.query(sql,  { replacements: { wallet: config.wallet, description: credit["http://purl.org/dc/terms/description"] } }).then(function(res) {
    return res;
  }).catch(function(err){
...
function getBalance(source, sequelize, config, callback) {
  ...
  var balanceSql = 'Select sum(amount) amount from Ledger where source = :source and wallet = :wallet ;';

  sequelize.query(balanceSql,  { replacements: { wallet: config.wallet, source: source } }).then(function(res) {
    return res;
  }).catch(function(err){
 ...
 * @param  {Function} callback callback
 */
function genesis(config, callback) {
  // vars
  var sequelize;
...
function today(source, sequelize, config, callback) {
  ...
  var coinbaseSql = 'Select sum(amount) amount from Credit where destination = :source and wallet = :wallet and DATE(`timestamp`) = CURDATE() ;';
...
function insert(credit, sequelize, config, callback) {
  ...
  created: credit["http://purl.org/dc/terms/created"], "destination" : credit["https://w3id.org/cc#destination"], wallet: config.wallet } }).then(function(res) {
    ...
    if (credit["http://purl.org/dc/terms/description"]) insertSql += ", `description`";
