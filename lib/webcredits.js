...
function deposit(credit, sequelize, config, callback) {
  ...
  sequelize.query(depositSql,  { replacements: { wallet: config.wallet, source: credit["https://w3id.org/cc#source"], amount: credit["https://w3id.org/cc#amount"], currency: credit["https://w3id.org/cc#currency"] } }).then(function(res) {
...
function toledger(credit, sequelize, config, callback) {
  ...
  sequelize.query(ledgerSql,  { replacements: { wallet: config.wallet, source: credit["https://w3id.org/cc#source"], amount: credit["https://w3id.org/cc#amount"], currency: credit["https://w3id.org/cc#currency"] }   }).then(function(){
    console.log('Deposit successful!');
  }).catch(function(err){
    console.log('Deposit Failed.', err);
    callback(err);
